...
local Sub_op, parent = torch.class('nc.Sub_op', 'nc.Abstract_op', ops)
ops.nb_existing_ops = ops.nb_existing_ops + 1

function Sub_op:__init(memory_size)
	parent.__init(self, memory_size)
end

function Sub_op:updateOutput(input)
...
function Sub_op:updateGradInput(input, gradOutput)
    local dist1 = input[1][1]
    local dist2 = input[1][2]
    ...
    for i=1,dist1:size(1) do
        ...
        for c=1,gradOutput:size(1) do
            ...
            acc_1 = acc_1 + gradOutput[c]*dist2[(i-c)%self.memory_size+1]
            -- dist2[(c-i)%self.memory_size+1] = sum_b dist2[b] 1_{a-i==c}
            acc_2 = acc_2 + gradOutput[c]*dist1[(c+i-2)%self.memory_size+1]
        end
